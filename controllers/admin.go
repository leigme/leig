...
func (this *AdminController) ActionAddArticle() {
	...
	if Title != "" && Content != "" && ArticleSortId != "" {
		...
		erraa := models.AddArticle(Title, Content, id)
		erruc := models.UpdateArticleSortCount(id)
		if erraa != nil {
			erraa.Error()
			this.Redirect("/", 302)
			return
...
func (this *AdminController) ActionAddPhoto() {
	...
	if Titile != "" && Url != "" && Psi != "" {
		id, _ := strconv.ParseInt(Psi, 10, 64)
		errap := models.AddPhoto(Titile, Url, id)
		if errap != nil {
			errap.Error()
			this.Redirect("/", 302)
			return
		}
		erruc := models.UpdataPhotoSortCount(id)
		if erruc != nil {
			erruc.Error()
			this.Redirect("/", 302)
			return
